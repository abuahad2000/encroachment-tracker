...
def main():
    print("🚀 بدء إنشاء التقرير التنفيذي الشامل للبلاغات المعلقة (Excel)...")
    ...
    # عنوان التقرير
    ws_kpi.merge_cells("B2:M2")
    ws_kpi.row_dimensions[2].height = 42
    title_cell = ws_kpi["B2"]
    title_cell.value = "شركة المياه الوطنية • قطاع المشاريع الراسمالية بالقطاع الأوسط | التقرير التنفيذي الشامل لحوكمة بلاغات التعديات المعلقة"
    ...
    ws_kpi.merge_cells("B3:M3")
    ws_kpi.row_dimensions[3].height = 24
    sub_cell = ws_kpi["B3"]
    sub_cell.value = f"تاريخ الاستخراج والتحليل: 19 سبتمبر 2026 | النطاق: بلاغات مشاريع البنية التحتية الجارية | إجمالي البلاغات المعلقة المعتمدة: {len(pending_assigned)} بلاغاً"
    ...
    # بطاقات المؤشرات الرقمية (KPI Cards)
    kpis = [
        ("إجمالي المعلق بالمشاريع", f"{len(pending_assigned)} بلاغاً", C_ROYAL_BLUE, "مشاريع جارية"),
        ("خرق جسيم (>180 يوم)", f"{sum(1 for r in pending_assigned if (r.get('ageDays') or 0) > 180)} بلاغاً", "831843", "أولوية قصوى 🚨"),
        ("حرج وتشغيلي (91-180)", f"{sum(1 for r in pending_assigned if 90 < (r.get('ageDays') or 0) <= 180)} بلاغاً", "991B1B", "تجاوز SLA 🔴"),
        ("متأخر عن SLA (61-90)", f"{sum(1 for r in pending_assigned if 60 < (r.get('ageDays') or 0) <= 90)} بلاغاً", "9A3412", "إنذار أولي 🟠"),
        ("قيد المتابعة (0-60)", f"{sum(1 for r in pending_assigned if (r.get('ageDays') or 0) <= 60)} بلاغاً", "166534", "ضمن المعالجة 🟢"),
    ...
    ws_kpi.merge_cells("B9:M9")
    ws_kpi.row_dimensions[9].height = 28
    sec1 = ws_kpi["B9"]
    sec1.value = "📌 جدول تفصيلي: موقف مدراء البرامج ومعدلات التأخير للبلاغات المعلقة المعتمدة"
    ...
    headers_mgr = [
        "م", "مدير البرنامج المسؤول", "القطاع والنطاق التشغيلي", "البلاغات المعلقة", 
    ...
    curr_row = 11
    # نحسب إحصائيات كل مدير برنامج من واقع الـ 49 بلاغ
    mgr_stats_map = {}
    for r in pending_assigned:
        m_name = r['project'].get('programManager', 'غير محدد')
        if m_name not in mgr_stats_map:
            mgr_stats_map[m_name] = {'total': 0, 'b0_30': 0, 'b31_60': 0, 'b61_90': 0, 'b91_180': 0, 'b_gt180': 0, 'ages': []}
    ...
    # ترتيب المدراء: أصحاب البلاغات المعلقة أولاً ثم البقية
    all_ten_managers = [
    ...
    for idx, mgr in enumerate(all_ten_managers, start=1):
        ...
        st = mgr_stats_map.get(mgr['name'], {'total': 0, 'b0_30': 0, 'b31_60': 0, 'b61_90': 0, 'b91_180': 0, 'b_gt180': 0, 'ages': []})
        ...
        for col_i, v in enumerate(row_vals, start=2):
            ...
            c.font = Font(name="Segoe UI", size=9, bold=(col_i in [3, 5]), color=C_DARK_TEXT)
            ...
            # تمييز خلايا الخطر
            if col_i == 5 and st['total'] > 0:  # عمود إجمالي البلاغات المعلقة
                c.font = Font(name="Segoe UI", size=10, bold=True, color=C_ROYAL_BLUE)
            elif col_i == 10 and st['b_gt180'] > 0: # عمود الخرق الجسيم >180
    ...
    total_cells = [
        "الإجمالي", "", "كافة قطاعات مدينة الرياض والمحافظات", len(pending_assigned),
        sum(1 for r in pending_assigned if (r.get('ageDays') or 0) <= 30),
        sum(1 for r in pending_assigned if 30 < (r.get('ageDays') or 0) <= 60),
        sum(1 for r in pending_assigned if 60 < (r.get('ageDays') or 0) <= 90),
        sum(1 for r in pending_assigned if 90 < (r.get('ageDays') or 0) <= 180),
        sum(1 for r in pending_assigned if (r.get('ageDays') or 0) > 180),
    ...
    # جدول 2: المقاولون الأكثر تأخيراً في معالجة بلاغات التعديات
    curr_row += 3
    ws_kpi.merge_cells(f"B{curr_row}:M{curr_row}")
    ws_kpi.row_dimensions[curr_row].height = 26
    sec2 = ws_kpi[f"B{curr_row}"]
    sec2.value = "⚠️ جدول متابعة المقاولين: قائمة المقاولين الأكثر تسجيلاً للبلاغات المعلقة بالمشاريع"
    ...
    curr_row += 1
    headers_contractor = [
        "م", "اسم المقاول المنفذ", "مدير البرنامج المشرف", "البلاغات المعلقة",
        "النسبة من المعلق", "أقدم بلاغ (يوم)", "الحالة الأكثر تكراراً", "الإجراء الميداني المطلوب والتوجيه"
    ]
    ws_kpi.row_dimensions[curr_row].height = 26
    ws_kpi.merge_cells(f"I{curr_row}:M{curr_row}")

    ws_kpi.cell(row=curr_row, column=2, value="م").fill = PatternFill(start_color=C_PRIMARY_NAVY, end_color=C_PRIMARY_NAVY, fill_type="solid")
    ws_kpi.cell(row=curr_row, column=3, value="اسم المقاول المنفذ").fill = PatternFill(start_color=C_PRIMARY_NAVY, end_color=C_PRIMARY_NAVY, fill_type="solid")
    ws_kpi.cell(row=curr_row, column=4, value="مدير البرنامج المشرف").fill = PatternFill(start_color=C_PRIMARY_NAVY, end_color=C_PRIMARY_NAVY, fill_type="solid")
    ws_kpi.cell(row=curr_row, column=5, value="البلاغات المعلقة").fill = PatternFill(start_color=C_PRIMARY_NAVY, end_color=C_PRIMARY_NAVY, fill_type="solid")
    ws_kpi.cell(row=curr_row, column=6, value="النسبة من المعلق").fill = PatternFill(start_color=C_PRIMARY_NAVY, end_color=C_PRIMARY_NAVY, fill_type="solid")
    ws_kpi.cell(row=curr_row, column=7, value="أقدم بلاغ").fill = PatternFill(start_color=C_PRIMARY_NAVY, end_color=C_PRIMARY_NAVY, fill_type="solid")
    ws_kpi.cell(row=curr_row, column=8, value="حالة المعالجة الغالبة").fill = PatternFill(start_color=C_PRIMARY_NAVY, end_color=C_PRIMARY_NAVY, fill_type="solid")
    c_act = ws_kpi.cell(row=curr_row, column=9, value="الإجراء التنفيذي المطلوب والتوجيه الإداري")
    ...
    for r in pending_assigned:
        ...
        if c_name not in contractors_map:
            contractors_map[c_name] = {'count': 0, 'managers': set(), 'ages': [], 'statuses': {}}
        contractors_map[c_name]['count'] += 1
    ...
    sorted_contractors = sorted(contractors_map.items(), key=lambda x: -x[1]['count'])
    ...
    for c_idx, (c_name, c_data) in enumerate(sorted_contractors, start=1):
        ...
        pct = f"{(c_data['count'] / len(pending_assigned) * 100):.1f}%"
        ...
        # إجراء تنفيذي للمقاول
        if max_age > 180:
            c_action = f"تطبيق اشتراطات العقد وغرامات تعطيل المرافق فوراً لإغلاق البلاغ المتأخر ({max_age} يوماً)"
        elif c_data['count'] >= 5:
            c_action = f"عقد اجتماع تنسيقي عاجل مع الإدارة الفنية للمقاول وحصر جميع الملاحظات المشتركة"
        else:
            c_action = "إلزام المقاول بالمعالجة الميدانية والتنسيق مع الجهة المتضررة لرفع محضر الإنجاز"
        ...
        ws_kpi.cell(row=curr_row, column=5, value=c_data['count'])
        ws_kpi.cell(row=curr_row, column=6, value=pct)
        ws_kpi.cell(row=curr_row, column=7, value=f"{max_age} يوم")
        ws_kpi.cell(row=curr_row, column=8, value=top_status)
        ws_kpi.cell(row=curr_row, column=9, value=c_action)

        for ci in range(2, 14):
            cell = ws_kpi.cell(row=curr_row, column=ci)
            cell.font = Font(name="Segoe UI", size=9, bold=(ci in [3, 5]), color=C_DARK_TEXT)
            cell.alignment = Alignment(horizontal="right" if ci in [3, 4, 9] else "center", vertical="center")
    ...
    ws_kpi.column_dimensions['F'].width = 14
    ws_kpi.column_dimensions['G'].width = 14
    ws_kpi.column_dimensions['H'].width = 15
    ws_kpi.column_dimensions['I'].width = 15
    ws_kpi.column_dimensions['J'].width = 15
    ws_kpi.column_dimensions['K'].width = 15
    ws_kpi.column_dimensions['L'].width = 16
    ...
    # عنوان الورقة
    ws_rep.merge_cells("A1:Z1")
    ws_rep.row_dimensions[1].height = 36
    h_rep = ws_rep["A1"]
    h_rep.value = "سجل بلاغات التعدي المعلقة المعتمدة لقطاع المشاريع الجارية (NWC) - موجهة لمدراء البرامج والمقاولين"
    ...
    ws_rep.merge_cells("A2:Z2")
    ws_rep.row_dimensions[2].height = 22
    h_sub = ws_rep["A2"]
    h_sub.value = f"تاريخ التدقيق: 19 سبتمبر 2026 | الربط المكاني والتعاقدي 100% | إجمالي البلاغات: {len(pending_assigned)} بلاغاً موزعة على 7 مدراء برامج معنيين"
    ...
    for col_i, h in enumerate(headers_reports, start=1):
        cell = ws_rep.cell(row=4, column=col_i, value=h)
        cell.font = Font(name="Segoe UI", size=9, bold=True, color=C_WHITE)
        cell.alignment = Alignment(horizontal="center", vertical="center", wrap_text=True)
    ...
    # ملء البلاغات الـ 49
    for r_idx, rep in enumerate(pending_assigned, start=1):
        row_num = r_idx + 4
        ws_rep.row_dimensions[row_num].height = 24
        ...
        for col_i, val in enumerate(row_data, start=1):
            cell = ws_rep.cell(row=row_num, column=col_i, value=clean_val(val))
            cell.font = Font(name="Segoe UI", size=9, color=C_DARK_TEXT)
            cell.border = thin_border()

            # محاذاة وتلوين حسب العمود
            ...
            # تلوين خاص للأعمدة الحيوية
            if col_i == 2: # رقم البلاغ
                cell.font = Font(name="Segoe UI", size=9, bold=True, color=C_ROYAL_BLUE)
            elif col_i == 3: # مدير البرنامج
                cell.font = Font(name="Segoe UI", size=9, bold=True, color=C_PRIMARY_NAVY)
            elif col_i == 16: # عمر البلاغ بالأيام
                cell.font = Font(name="Segoe UI", size=9, bold=True)
            elif col_i == 17: # شريحة العمر
                ...
                cell.alignment = Alignment(horizontal="center", vertical="center")
            elif col_i == 26: # الإجراء المطلوب
                cell.font = Font(name="Segoe UI", size=8.5, bold=True, color='831843' if age_days > 180 else '0F3B66')

    # تجميد الصفوف العلوية وتفعيل التصفية
    ws_rep.freeze_panes = "A5"
    ws_rep.auto_filter.ref = f"A4:Z{len(pending_assigned) + 4}"
    auto_fit_columns(ws_rep, max_cols=26, max_width_limit=45)
    ws_rep.column_dimensions['A'].width = 5
    ws_rep.column_dimensions['B'].width = 14
    ws_rep.column_dimensions['C'].width = 22
    ws_rep.column_dimensions['D'].width = 38
    ws_rep.column_dimensions['G'].width = 28
    ws_rep.column_dimensions['Q'].width = 26
    ws_rep.column_dimensions['R'].width = 22
    ws_rep.column_dimensions['T'].width = 36
    ws_rep.column_dimensions['W'].width = 40
    ws_rep.column_dimensions['X'].width = 35
    ws_rep.column_dimensions['Y'].width = 40
    ws_rep.column_dimensions['Z'].width = 42

    # -------------------------------------------------------------
    # 3. ورقات عمل منفصلة لكل مدير برنامج من الـ 7 مدراء
    # -------------------------------------------------------------
    active_managers_list = [
        ("أمجد الفالح", "صرف صحي - غرب الرياض", "555390254", "aalfaleh@nwc.com.sa"),
        ("عبدالله الأسود العنزي", "مياه وصرف - استثناء شامل كامل الرياض", "509997997", "amalenazi@nwc.com.sa"),
        ("عسكر لسلوم", "صرف صحي - شمال الرياض", "567448829", "llslum@nwc.com.sa"),
    ...
    for mgr_name, mgr_scope, mgr_phone, mgr_email in active_managers_list:
        mgr_reports = [r for r in pending_assigned if r['project'].get('programManager') == mgr_name]
        if not mgr_reports:
            continue
        ...
        # عنوان ورقة مدير البرنامج
        ws_m.merge_cells("A1:S1")
        ws_m.row_dimensions[1].height = 36
        h_m = ws_m["A1"]
        h_m.value = f"تقرير بلاغات التعدي المعلقة الخاصة بمدير البرنامج: {mgr_name}"
        ...
        ws_m.merge_cells("A2:S2")
        ws_m.row_dimensions[2].height = 22
        h_ms = ws_m["A2"]
        h_ms.value = f"النطاق: {mgr_scope} | الجوال: {mgr_phone} | البريد: {mgr_email} | إجمالي البلاغات المعلقة: {len(mgr_reports)} بلاغاً"
        ...
        headers_m = [
            "م", "رقم البلاغ", "اسم المشروع", "رقم العملية", "أمر الشراء (PO)",
        ...
        ws_m.auto_filter.ref = f"A4:S{len(mgr_reports) + 4}"
        auto_fit_columns(ws_m, max_cols=19, max_width_limit=45)
        ws_m.column_dimensions['A'].width = 5
        ws_m.column_dimensions['B'].width = 14
        ws_m.column_dimensions['C'].width = 38
        ws_m.column_dimensions['F'].width = 28
        ws_m.column_dimensions['M'].width = 25
        ws_m.column_dimensions['N'].width = 22
        ws_m.column_dimensions['Q'].width = 45
        ws_m.column_dimensions['R'].width = 40
    ...
    h_as = ws_all["A2"]
    h_as.value = f"إجمالي البلاغات المعلقة غير المغلقة: {len(all_pending)} بلاغاً (49 مشاريع جارية معتمدة + 2,375 تشغيل وصيانة وشبكات قائمة ومستبعدة)"
    ...
    headers_all = [
        "م",
        "رقم البلاغ",
        "مسار التوجيه والقطاع",
        "الجهة / مدير البرنامج المسؤول",
        "اسم المشروع / الشبكة",
        "المقاول المسجل بالبلاغ",
        "المدينة",
        "الحي",
        "الشارع",
        "تاريخ البلاغ",
        "عمر البلاغ (يوم)",
        "شريحة العمر والمخاطر",
        "حالة البلاغ في المنظومة",
        "الجهة المالكة للمرفق",
        "الجهة المتعدية",
        "وصف التعدي",
